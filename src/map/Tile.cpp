...
std::size_t Tile::rawBlobSize() const {
...
std::size_t Tile::rgbaBlobSize() const {
...
void Tile::stbLoad() const {
    ...
    const auto ptr{stbi_load_from_memory(reinterpret_cast<stbi_uc const*>(_rawBlob.data()), static_cast<int>(_rawBlob.size()), &_pxW, &_pxH,
