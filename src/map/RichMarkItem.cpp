...
void MarkItem::paint() {
    if (_style.markerEnabled) {
        ImPlot::SetNextMarkerStyle(_style.markerType, _style.markerSize, _style.markerFill, _style.markerWeight, _style.markerOutline);
...
void MarkItem::updateRadiusPoints() {
    ...
    const double r{sqrt(std::pow(x - _osmCoords.x, 2.0) + std::pow(y - _osmCoords.y, 2.0))};
